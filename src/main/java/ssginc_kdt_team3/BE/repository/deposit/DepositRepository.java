...
public interface DepositRepository extends JpaRepository<Deposit, Long> {
    ...
    //=============================================== 가게의 예약금 목록 조회 ===============================================
    @Query("select d from Deposit d inner join Reservation r on d.reservation.id = r.id inner join Shop s on r.shop.id = s.id where s.id = :shopId order by d.id desc")
    List<Deposit> findShopDepositList(@Param("shopId") Long shopId);

    //=============================================== 지점의 예약금 목록 조회 ===============================================
    @Query("select d from Deposit d where d.reservation.shop.branch.id = :branchId order by d.id desc")
