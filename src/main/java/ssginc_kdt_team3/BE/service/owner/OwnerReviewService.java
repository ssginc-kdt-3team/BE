...
public class OwnerReviewService {
  ...
  public Page<OwnerReviewListDTO> getReviewList(String type, Long ownerId, Pageable pageRequest, LocalDateTime start, LocalDateTime end) {
    ...
    if(type.equals("pointDesc")){
      Page<Review> pointDesc = reviewRepository.findAllByReservation_Shop_Owner_IdOrderByPointDesc(ownerId, pageRequest);
      return convertDto(pointDesc);

    } else if (type.equals("pointAsc")){
      Page<Review> pointAsc = reviewRepository.findAllByReservation_Shop_Owner_IdOrderByPointAsc(ownerId, pageRequest);
